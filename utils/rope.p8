...
function _rope:reset_pos(pos)
  ...
  for i = 1,self.size do
    ...
    self.particles[i]={curr_pos=p, last_pos=p}
  end
end

function _rope:draw()
  for i = 2, #self.particles do
    local a = self.particles[i-1].curr_pos
    local b = self.particles[i].curr_pos
...
function _rope:update()
  ...
  for particle in all(self.particles) do
    local init_pos = particle.curr_pos:copy()
    particle.curr_pos += particle.curr_pos - particle.last_pos + self.acc
  ...
  for _ = 1,flag_num_constraint_relaxations do
    for i = 2,#self.particles do
      ...
      if i > 2 then
        self.particles[i-1].curr_pos += diff_delta
        self.particles[i].curr_pos -= diff_delta
      else
        self.particles[i].curr_pos -= diff_delta * 2
      end
    end
  end
  -- reset anchor particle
  self.particles[1].curr_pos = self.pos
end

function _rope:aabb()
  local first_pos = self.particles[1].curr_pos
  local last_pos = self.particles[#self.particles].curr_pos
